---
name: task-create
description: Create a structured task card in vAIbe-OS with interactive user confirmation. Triggers: task, create task, new task, задача, создать задачу.
license: MIT
---
...
- Assignee / Ответственный (who will execute the task)
- Proposed title (agree / suggest own)

Use the **structured-question tool** to gather these as finite choices (Cursor `AskQuestion`, Claude Code `AskUserQuestion`, or the IDE equivalent — see `.vaibe/rules/interactive-patterns.md`). **Actually invoke the tool** — do not just describe the options in text. If no structured tool is available, fall back to a numbered list in plain text.
...
**If visibility = личный**: (1) если имя проекта попадает под безличные паттерны корневого `.gitignore` (соглашение о пути, например `*Личное*` — см. `.vaibe/rules/git-cross-platform.md`), отдельная запись не нужна; (2) иначе добавить `/Проекты/{NAME}/` в **`.git/info/exclude`** (локально, не коммитится), а не в трекаемый `.gitignore`, чтобы не светить имя в репозитории.
...
**Sanitize the task title before using it as a folder name (Windows compatibility):**
The folder `{NUM}-{TITLE}` must be safe on all platforms. Before constructing the path, replace any Windows-forbidden characters in `{TITLE}`:
- `:` → `-` (most common issue — colons in Russian titles like «Задача: фиксация»)
- `<`, `>`, `"`, `\`, `|`, `?`, `*` → remove or replace with `-`
- `/` → `-` (slash creates nested directories on all OS)

Apply this silently — do not mention the replacement to the user unless the title was significantly changed.
...
- [ ] For личный projects: исключение настроено (паттерн в `.gitignore` или `/Проекты/{NAME}/` в `.git/info/exclude`)
