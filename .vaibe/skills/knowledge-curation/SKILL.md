...
description: How to maintain and curate the knowledge base without degradation. Reference material (non-actionable knowledge).
license: MIT
---

# Knowledge Curation Guide — How to Maintain the Knowledge Base

Meta-knowledge file that defines how to add, update, and maintain references in `.vaibe/skills/`. Ensures consistency, quality, and alignment with vAIbe-OS ontology across all knowledge files.

## When to use

- Adding a new knowledge file to `.vaibe/skills/`
...
- Running the `evolve` skill when it involves knowledge changes (see `.vaibe/skills/evolve/SKILL.md`)
...
|---|---|---|---|
| `.vaibe/skills/` | System-level reference materials | Agent + human | `/evolve` skill |
...
| `.vaibe/skills/` | Actionable playbooks | Agent | `/evolve` skill |
| `.vaibe/rules/` | Behavioral constraints | Agent | `/evolve` skill |
...
Every file in `.vaibe/skills/` must follow this template:
...
- {Situation 2}
- {Links to relevant skills: `skill-name` in .vaibe/skills/}
...
1. Does this knowledge already exist in `.vaibe/skills/`? → Cross-reference instead
2. Is this actionable procedure? → Create a skill in `.vaibe/skills/`, not knowledge
3. Is this project-specific? → Put it in `Проекты/{NAME}/База знаний/`
4. Is this personal to the user? → Put it in `База знаний/`
5. Will this be useful across multiple projects and contexts? → `.vaibe/skills/` is the right place
...
- [ ] Language is English (matching `.vaibe/` convention)
...
If the new file introduces terms not yet in `.vaibe/skills/glossary/SKILL.md`, add them with:
- English term
- Russian translation
- Concise definition
- Reference to the new knowledge file

This is enforced by **Guard G9** (`Guards invariants`) — the agent will warn about missing glossary entries.

### Step 6 — Link from skills

If the new knowledge file should be referenced during skill execution (skill discovery is by
`description`, there is no separate router):
- Update `.vaibe/skills/` Knowledge Base table
- Add a `## Related knowledge` entry in relevant domain skills (`.vaibe/skills/`)
...
2. Check against Ontology (`ONTOLOGY.md`) and Manifesto (`MANIFESTO.md`) — see `.vaibe/skills/evolve/SKILL.md`
3. Add new information rather than replacing (document evolution)
4. Update the Sources section with new references and dates
5. Update `.vaibe/skills/glossary/SKILL.md` if terminology changed
...
**Guard G8** (`Guards invariants`) enforces freshness: files not modified for 18+ months are flagged during `weekly-review`.
...
Knowledge creation and update is a form of system evolution governed by the `/evolve` skill (`.vaibe/skills/evolve/SKILL.md`). When using `/evolve` for knowledge changes:
...
- vAIbe-OS Ontology: `.vaibe/rules/ontology.md` — five laws governing system evolution
- vAIbe-OS Manifesto: `.vaibe/rules/manifesto.md` — behavioral principles
- Evolve skill: `.vaibe/skills/evolve/SKILL.md` — procedure for system self-improvement
- GUARDS: `Guards invariants` — machine-checkable integrity rules (G7: knowledge protection, G8: freshness check, G9: glossary sync)
