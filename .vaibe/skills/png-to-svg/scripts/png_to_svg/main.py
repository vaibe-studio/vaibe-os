...
Run (uv project):
  ...
  uv run --project $P $P/main.py embed image.png output.svg
  uv run --project $P --extra trace $P/main.py trace infographic.png infographic.svg
  uv run --project $P $P/main.py mono logo.png logo.svg
"""
...
def mode_trace(input_path: str, output_path: str) -> None:
    ...
    try:
        ...
        sys.exit(
            "[png_to_svg] vtracer not installed. Re-run with the 'trace' extra: "
            "uv run --project <png_to_svg dir> --extra trace <png_to_svg dir>/main.py ..."
        )
    ...
    if inp.suffix.lower() in (".jpg", ".jpeg"):
        try:
            ...
            sys.exit(
                ...
                "Re-run with the 'trace' extra (--extra trace)."
            )
